...
class Warrior:
    ...
    def refresh_stats(self):
        ...
        self.rank = ranks[(self.level - (self.level%10))] if self.level >= 10 else "Pushover"
        
    def new_achievement(self, description):
    ...
    def battle(self, enemy_level):
        ...
        if enemy_level == self.level:
            ...
            if enemy_level >= 10 and (enemy_level - (enemy_level % 10) >= self.level and enemy_level - 5 >= self.level):
    ...
    def training(self, array):
        if len(array) != 3:
            return "Error"
        if array[2] <= self.level:
            self.experience += array[1] 
            self.refresh_stats()
            self.new_achievement(array[0])
            return array[0]
        else:
